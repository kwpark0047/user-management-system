...
router.get('/qr/:qrCode', (req, res) => {
  try {
    ...
    if (!table.store_active || !table.is_active) return res.status(400).json({ error: '현재 이용할 수 없는 테이블입니다' });
    res.json(table);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
// 테이블 생성 (table:manage 권한 - owner/admin만)
router.post('/', authMiddleware, (req, res) => {
  try {
    const { store_id, name } = req.body;
    if (!store_id || !name) return res.status(400).json({ error: '매장 ID와 테이블명은 필수입니다' });
...
// 테이블 수정 (table:manage 권한)
router.put('/:id', authMiddleware, (req, res) => {
  try {
    const table = Table.findById(req.params.id);
    if (!table) return res.status(404).json({ error: '테이블을 찾을 수 없습니다' });
    const role = getStoreRole(req.user.id, table.store_id);
    if (!role || !['owner', 'admin'].includes(role)) {
      return res.status(403).json({ error: '권한이 없습니다' });
    }
